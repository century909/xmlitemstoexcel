...
export default function XmlExtractor({ onBackToHome }: XmlExtractorProps) {
  ...
  const fetchGmailAttachments = async (token: string, month: string, company: string) => {
    ...
    try {
      ...
      if (allItems.length > 0) {
        ...
        // Save to localStorage for MenuCalculator
        try {
          localStorage.setItem(`consumo_data_${month}`, JSON.stringify(aggregated));
        } catch (e) {
          console.error("Error saving consolidated items to localStorage", e);
        }
      } else {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (allItems.length > 0) {
      ...
      // Save to localStorage for MenuCalculator
      try {
        localStorage.setItem(`consumo_data_${selectedMonth}`, JSON.stringify(aggregated));
      } catch (e) {
        console.error("Error saving consolidated items to localStorage", e);
      }
    } else {
  ...
  return (
    <div className="extractor-view">
      <div className="view-header">
        ...
        <h2>Extractor de Ítems de Facturas XML</h2>
        <p className="subtitle text-light">Importa tus XML (locales o de Gmail) para consolidar los consumos y precios promedio del mes.</p>
      </div>
      ...
      {items.length > 0 ? (
        ...
        !isLoading && (
          <div className="empty-state">
            <h3>Comienza extrayendo los ítems de tus facturas</h3>
            <p>Carga tus archivos .xml o búscalos en Gmail. La aplicación sumará las cantidades y costos de cada producto encontrado en todas las facturas del mes.</p>
          </div>
        )
      )}
    </div>
